...
def test_relational_gate_uses_eqr_identity_not_flat_eq_identity():
    from flopo2.ids.registry import relational_signature
    from flopo2.verify.gates import Combination, check_assertion

    assertion = {
        "po_id": "PO_0025034",
        "pato_id": "PATO_0000320",
        "source_text": "green leaves with red trichomes",
        "composition": {"status": "accept"},
        "part_restrictions": [
            {
    ...
    combinations = {
        ("PO_0025034", "PATO_0000320"): Combination("allowed", "test"),
        ("PO_0000282", "PATO_0000322"): Combination("allowed", "test"),
    }
    expected = relational_signature(
        "PO_0025034",
        "PATO_0000320",
        (("BFO_0000051", "PO_0000282", ("PATO_0000322",)),),
    )
    flat_only = check_assertion(
        assertion["source_text"],
        assertion,
        combinations,
        registry={
            ("PO_0025034", "PATO_0000320"): ("http://example.org/FLOPO_flat", False)
        },
        signature_registry={
            "EQ|PO_0025034|PATO_0000320": ("http://example.org/FLOPO_flat", False)
        },
    )
    assert flat_only.flopo_signature == expected
    assert flat_only.flopo_status == "new_class_candidate"
    assert not flat_only.flopo_iri

    exact_relational = check_assertion(
        assertion["source_text"],
        assertion,
        combinations,
        registry={},
        signature_registry={expected: ("http://example.org/FLOPO_relational", False)},
    )
    assert exact_relational.flopo_status == "existing"
    assert exact_relational.flopo_iri.endswith("FLOPO_relational")


def test_relational_gate_checks_nested_pairs_and_fails_closed_on_invalid_grammar():
    from flopo2.verify.gates import Combination, check_assertion
    ...
    outer = {("PO_0025034", "PATO_0000320"): Combination("allowed", "test")}
    novel = check_assertion(assertion["source_text"], assertion, outer)
    assert novel.status == "review"
    assert "nested_po_pato_novel" in novel.reasons

    blocked = check_assertion(
        assertion["source_text"],
        assertion,
        {
            **outer,
            ("PO_0000282", "PATO_0000322"): Combination("blocked", "test"),
        },
    )
    assert blocked.status == "blocked"
    assert "nested_po_pato_blocklisted" in blocked.reasons
    ...
    invalid = check_assertion(malformed["source_text"], malformed, outer)
    assert invalid.status == "blocked"
    assert "invalid_part_restriction" in invalid.reasons


def test_relational_gate_blocks_nested_ids_missing_from_live_catalogs():
    from flopo2.verify.gates import Combination, check_assertion
    ...
    assertion = {
        ...
        "part_restrictions": [
            {
                "property": "BFO_0000051",
                "filler_class": "PO_9999999",
                "qualities": ["PATO_9999999"],
            }
        ],
    }
    decision = check_assertion(
        ...
        {
            ("PO_0025034", "PATO_0000320"): Combination("allowed", "test"),
            # A stale combination row must never make absent ontology terms admissible.
            ("PO_9999999", "PATO_9999999"): Combination("allowed", "test"),
        },
        pato_catalog_ids={"PATO_0000320"},
        po_catalog_ids={"PO_0025034"},
    )
    assert decision.status == "blocked"
    assert "unknown_nested_po_id" in decision.reasons
    assert "unknown_nested_pato_id" in decision.reasons
...
def test_gate_disjunction_gate_is_attribute_aware():
    """Curator decision 2026-09-18 (C3): an "or" governing a different attribute than the
    asserted quality must not block that quality's assertion, but same-family disjunctions and
    ungroundable operands must still block, exactly as before."""

    from flopo2.verify.gates import check_assertion

    # Tribulus cistoides: "Annual or rarely biennial yellow-green, glabrescent herb;" -- the
    # disjunction is about life span (annual/biennial), not the colour "yellow-green".
    combo = {("PO_0000003", "PATO_0001941"): type("C", (), {"status": "allowed"})()}
    life_span_disjunction = check_assertion(
        "Annual or rarely biennial yellow-green, glabrescent herb;",
        {
            "po_id": "PO_0000003",
    ...
    assert life_span_disjunction.status == "accepted"
    assert "atomic_clause_contains_disjunction" not in life_span_disjunction.reasons

    # "white or pink" is still a real colour-vs-colour disjunction and must still block.
    colour_combo = {("PO_petal", "PATO_0000323"): type("C", (), {"status": "allowed"})()}
    colour_disjunction = check_assertion(
    ...
    assert colour_disjunction.status == "review"
    assert "atomic_clause_contains_disjunction" in colour_disjunction.reasons

    # "glabrous or pubescent" is a real pilosity-vs-pilosity disjunction and must still block.
    pilosity_combo = {("PO_stem", "PATO_0000453"): type("C", (), {"status": "allowed"})()}
    pilosity_disjunction = check_assertion(
        "Stems glabrous or pubescent, terete.",
        {
            "po_id": "PO_stem",
            "pato_id": "PATO_0000453",
            "source_text": "glabrous",
            "source_start": 6,
            "source_end": 14,
        },
        pilosity_combo,
    )
    assert pilosity_disjunction.status == "review"
    assert "atomic_clause_contains_disjunction" in pilosity_disjunction.reasons
