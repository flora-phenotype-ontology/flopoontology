...
from flopo2.owl.build import PUBESCENCE_PHENOTYPE, build_ontology
...
def test_build_ontology_groups_botanical_pubescence(tmp_path):
    registry = tmp_path / "registry.tsv"
    registry.write_text(
        "flopo_iri\tflopo_num\tlabel\tsignature\tdeprecated\n"
        "http://purl.obolibrary.org/obo/FLOPO_0000001\t1\tleaf phenotype\t"
        "PHENO|PO_leaf\t0\n"
        "http://purl.obolibrary.org/obo/FLOPO_0000002\t2\tleaf pubescent hair\t"
        "EQ|PO_leaf|PATO_0001320\t0\n"
        "http://purl.obolibrary.org/obo/FLOPO_0980977\t980977\tpubescence phenotype\t"
        "EQ|PO_0025131|PATO_0001320\t0\n"
    )
    po = tmp_path / "po.tsv"
    po.write_text("id\tlabel\tsynonyms\tnamespace\nPO_leaf\tleaf\t\tplant_anatomy\n")
    pato = tmp_path / "pato.tsv"
    pato.write_text(
        "id\tlabel\tsynonyms\tslim\nPATO_0001320\tpubescent hair\t\t\n"
    )
    gated = tmp_path / "gated.jsonl"
    gated.write_text(
        json.dumps(
    ...
    build_ontology(gated, output, registry, po, pato)
    graph = Graph().parse(output)
    pubescent_leaf = URIRef("http://purl.obolibrary.org/obo/FLOPO_0000002")
