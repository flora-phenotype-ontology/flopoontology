...
def test_curator_reviewed_table_requires_known_decisions(tmp_path):
    mapping = tmp_path / "mapping.tsv"
    row = {field: "" for field in REPORT_FIELDS}
    row.update(
        {
    ...
    _write_tsv(
        proposals,
        PROPOSAL_FIELDS,
        [_proposal(curator_decision="accept", curator_notes="2026-09-18 curator approval")],
    )

    rows, summary = build_curation_table(
        mapping, proposals, evidence, curator_reviewed=True
    )
    assert rows[0]["curator_decision"] == "accept"
    assert summary["policy"]["curator_decision_default"] == "curator-recorded"

    _write_tsv(proposals, PROPOSAL_FIELDS, [_proposal()])
    with pytest.raises(ValueError, match="needs a known curator_decision"):
        build_curation_table(mapping, proposals, evidence, curator_reviewed=True)
