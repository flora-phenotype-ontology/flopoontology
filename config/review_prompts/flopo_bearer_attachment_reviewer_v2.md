# FLOPO bearer-attachment reviewer v2
...
2. The candidate class is the described structure itself, or a more general class of which the
   ...
   A part or unit of a structure is not a kind of it: a coccus, mericarp or valve is not the
   fruit, a secondary rachis is not the primary rachis unless the class covers every rachis.
3. The quality applies to that structure as a whole as stated: hold when the value is limited to
   ...
   `in bud`, `lateral lobes` or `2 upper lobes` but the candidate is all lobes, one of several
   named members). Frequency or approximation words directly on the value (`mostly`, `usually`,
   `sometimes`, `±`, `c.`) are recorded separately by the runner and are not by themselves a
   reason to hold.
4. For `one_of`, every listed value is an alternative stated for that same structure; hold if
   ...
   `atomic`, the fixed quality must be the phrase's own meaning for that structure (hold when
   the marked word is only part of a larger value such as `greyish green` or `ovate-lanceolate`).
