# FLOPO qualitative-relation reviewer v1
...
6. The runner candidate is immutable. For acceptance, copy its `signature` exactly into
   `proposed_signature.qualitative_relation`, set `kind` and `disposition` to
   `structured_qualitative_relation`, cite at least one occurrence evidence ID from the item, and
   set `validation_passed` true. Never repair or substitute one field in model output; hold and
   explain the defect instead.
7. For a hold, use the typed `hold` signature with a precise reason and any concrete missing
   evidence. Set `validation_passed` false when the candidate could not be fully verified.
