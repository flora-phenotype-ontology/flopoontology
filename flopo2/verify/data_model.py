...
def _pydantic_assertion(assertion: dict[str, Any]) -> TraitAssertion:
    ...
    payload = {
        ...
        "value_operands": assertion.get("value_operands") or None,
        "bearer_scope": assertion.get("bearer_scope") or None,
        "qualitative_value_relation": assertion.get("qualitative_value_relation") or None,
        "bearer_context_qualities": assertion.get("bearer_context_qualities") or None,
        "part_restrictions": assertion.get("part_restrictions") or None,
...
def _validate_assertion(
    ...
    # Qualitative relations and non-entailing (approximated) operands are structured
    # annotations without a FAC class expression.
    qualitative_relation_present = bool(
        assertion.get("qualitative_value_relation")
    ) or not operands_fac_representable(assertion)
    for relation_issue in (
        *validate_qualitative_value_relation(
            assertion,
            text,
            pato_catalog_ids=pato_ids,
            flopo_catalog_ids=flopo_bearer_ids,
        ...
        *(
            _PositionalIssue(code, message)
            for code, message in validate_positional(
                assertion, text, bspo_ids=load_bspo_ids(), check_po_closure=bool(po_ids)
            )
        ),
    ):
        _add(
            ...
            line_number,
            relation_issue.code,
            relation_issue.message,
            assertion_number,
        )

    stored_class_iri = parsed.phenotype_class_iri or ""
    if require_annotation_class and not stored_class_iri and not qualitative_relation_present:
        _add(
            issues,
            line_number,
    ...
    if stored_class_iri and not qualitative_relation_present:
    ...
    if statement_id and statement_span is not None:
        ...
        relation = parsed.qualitative_value_relation
        if relation is not None:
            evidence_items.extend(
                (
                    "qualitative_relation_evidence_outside_statement_interval",
                    "source_statement_omits_qualitative_relation_evidence",
                    text_value,
                    span,
                    False,
                )
                for text_value, span in (
                    (relation.from_text, (relation.from_start, relation.from_end)),
                    (
                        relation.connector_text,
                        (relation.connector_start, relation.connector_end),
                    ),
                    (relation.to_text, (relation.to_start, relation.to_end)),
                )
            )
        evidence_items.extend(
            (
                "value_operand_outside_statement_interval",
                "source_statement_omits_value_operand_evidence",
                operand_text,
                (operand_start, operand_end),
                False,
            )
            for operand_text, operand_start, operand_end in value_operand_spans(assertion)
        )
        evidence_items.extend(
            (
                "part_or_scope_cue_outside_statement_interval",
                "source_statement_omits_part_or_scope_cue",
                cue_text,
                (cue_start, cue_end),
                False,
            )
            for cue_text, cue_start, cue_end in part_evidence_spans(assertion)
        )
    ...
    bspo_ids = load_bspo_ids()
    for _depth, part in part_rows(assertion.get("part_restrictions", []) or []):
        raw_filler = str(part.get("filler_class", "") or "")
        filler = raw_filler.replace(":", "_")
        if PO_ID.fullmatch(filler):
            if po_ids and filler not in po_ids:
                _add(
                    issues,
                    line_number,
                    "unknown_part_restriction_po_id",
                    raw_filler,
                    assertion_number,
                )
        elif FLOPO_ID.fullmatch(filler):
            if flopo_bearer_ids and filler not in flopo_bearer_ids:
                _add(
                    issues,
                    line_number,
                    "unknown_part_restriction_flopo_filler",
                    raw_filler,
                    assertion_number,
                )
        elif not (BSPO_ID.fullmatch(filler) and (not bspo_ids or filler in bspo_ids)):
            _add(
                issues,
                line_number,
                (
                    "unknown_part_restriction_bspo_filler"
                    if BSPO_ID.fullmatch(filler)
                    else "invalid_part_restriction_filler"
                ),
                repr(raw_filler),
                assertion_number,
            )
        for quality in part.get("qualities", []) or []:
            normalized = str(quality).replace(":", "_")
            if PATO_ID.fullmatch(normalized):
                if pato_ids and normalized not in pato_ids:
                    _add(
                        issues,
                        line_number,
                        "unknown_part_restriction_pato_quality",
                        quality,
                        assertion_number,
                    )
            elif FLOPO_ID.fullmatch(normalized):
                if flopo_bearer_ids and normalized not in flopo_bearer_ids:
                    _add(
                        issues,
                        line_number,
                        "unknown_part_restriction_flopo_quality",
                        quality,
                        assertion_number,
                    )
            else:
                _add(
                    issues,
                    line_number,
                    "invalid_part_restriction_quality",
                    repr(quality),
                    assertion_number,
                )
        ...
        for line_number, line in enumerate(fh, 1):
            ...
            for assertion_number, assertion in enumerate(row_assertions):
                if (
                    ...
                    and operands_fac_representable(assertion)
                ):
                    try:
