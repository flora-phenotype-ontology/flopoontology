"""Apply externally reviewed assertion proposals to a flora JSONL artifact.
...
def _assertion_key(assertion: dict[str, Any]) -> tuple[Any, ...]:
    ...
    return (
        ...
        (
            relation.get("interpretation", ""),
            relation.get("from_value", ""),
            relation.get("to_value", ""),
        ),
    )
...
def _selector_key(selector: Any, proposal: dict[str, Any]) -> tuple[Any, ...]:
    ...
    # Removal selectors predate qualitative relations and may only target ordinary FAC/atomic
    # assertions. The empty relation component keeps their identity aligned with `_assertion_key`.
