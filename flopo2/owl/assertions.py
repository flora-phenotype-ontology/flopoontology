...
from flopo2.annotation.operands import operands_fac_representable, operands_make_qualified
...
ANNOTATION_PROPERTIES = {
    ...
    FLOPOANN.qualitative_from_value,
    FLOPOANN.qualitative_to_value,
    FLOPOANN.qualitative_from_text,
    FLOPOANN.qualitative_from_start,
    FLOPOANN.qualitative_from_end,
    FLOPOANN.qualitative_connector_text,
    FLOPOANN.qualitative_connector_start,
    FLOPOANN.qualitative_connector_end,
    FLOPOANN.qualitative_to_text,
    FLOPOANN.qualitative_to_start,
    FLOPOANN.qualitative_to_end,
    FLOPOANN.operand_index,
    FLOPOANN.operand_value,
    FLOPOANN.operand_text,
    FLOPOANN.operand_start,
    FLOPOANN.operand_end,
    FLOPOANN.operand_degree_qualifier,
    FLOPOANN.operand_value_qualifier,
    FLOPOANN.operand_frequency_qualifier,
    FLOPOANN.operand_qualifier_text,
    FLOPOANN.operand_qualifier_start,
    FLOPOANN.operand_qualifier_end,
    FLOPOANN.bearer_scope_outer_bearer,
    FLOPOANN.bearer_scope_class,
    FLOPOANN.bearer_scope_mode,
    FLOPOANN.bearer_scope_text,
    FLOPOANN.bearer_scope_start,
    FLOPOANN.bearer_scope_end,
    FLOPOANN.part_text,
}
...
def _curie_iri(value: object) -> URIRef | None:
    ...
    if ":" in text:
        ...
        if prefix.upper() in {"PO", "PATO", "UO", "ENVO", "FLOPO", "RO", "BFO", "BSPO"}:
    ...
    if re.match(r"^(?:PO|PATO|UO|ENVO|FLOPO|RO|BFO|BSPO)_", text):
...
def _compile_part_rows(graph: Graph, nodes: _Nodes, rows: list[dict]) -> list[BNode]:
    restrictions: list[BNode] = []
    for row in rows:
        prop = URIRef(row["property"])
        filler_class = URIRef(row["filler_class"])
        graph.add((prop, RDF.type, OWL.ObjectProperty))
        graph.add((filler_class, RDF.type, OWL.Class))
        filler_members: list = [filler_class]
        for quality in row["qualities"]:
            quality_iri = URIRef(quality)
            graph.add((quality_iri, RDF.type, OWL.Class))
            filler_members.append(_some(graph, nodes, HAS_QUALITY, quality_iri))
        # E3 nesting: a part of the part (abaxial epidermis has part trichome that is star shaped).
        filler_members.extend(
            _compile_part_rows(graph, nodes, row.get("part_restrictions", []) or [])
        )
        restrictions.append(
            _some(graph, nodes, prop, _intersection(graph, nodes, filler_members))
        )
    return restrictions


def _part_restrictions(graph: Graph, nodes: _Nodes, assertion: dict) -> list[BNode]:
    """Compile qualities on typed parts without moving them onto the outer bearer."""

    return _compile_part_rows(graph, nodes, canonical_part_restrictions(assertion))
...
def _phenotype_expression(
    ...
    nested_parts = _part_restrictions(graph, nodes, assertion)
    ...
    if negated and negation_scope == "quality":
        ...
        bearer = _intersection(
            graph, nodes, [po, negated_quality, *bearer_context, *nested_parts]
        )
        eq = _some(graph, nodes, HAS_PART, bearer)
    else:
        bearer = _intersection(
            graph, nodes, [po, *quality, *bearer_context, *nested_parts]
        )
...
def formal_assertion_iri(statement_id: str, index: int, assertion: dict) -> URIRef:
    ...
    if assertion.get("part_restrictions", []) or []:
        signature["part_restrictions"] = canonical_part_restrictions(assertion)
    if assertion.get("qualitative_value_relation"):
        signature["qualitative_value_relation"] = qualitative_relation_semantics(assertion)
...
def _strict(assertion: dict, unqualified_as_universal: bool) -> bool:
    if assertion.get("qualitative_value_relation"):
        return False
    # Entailing degree operands keep strict eligibility; operand-level frequency or
    # approximation makes the claim qualified (schema extension E2).
    if not operands_fac_representable(assertion) or operands_make_qualified(assertion):
        return False
...
def _logical_status(assertion: dict, strict: bool) -> str:
    if assertion.get("qualitative_value_relation"):
        return "structured_qualitative_relation"
    if not operands_fac_representable(assertion):
        return "structured_value_operands"
...
def _emit_operand(graph: Graph, subject: URIRef, prop: URIRef, iri: URIRef, operand: dict) -> None:
    """Reify one value operand (E2) with its grounded value, qualifiers and evidence."""

    graph.add((iri, RDF.type, OWL.NamedIndividual))
    graph.add((iri, RDF.type, FLOPOANN.ValueOperand))
    graph.add((subject, prop, iri))
    value = _curie_iri(operand.get("value"))
    if value is not None:
        graph.add((value, RDF.type, OWL.Class))
        graph.add((iri, FLOPOANN.operand_value, value))
    for key, prop_, default in (
        ("degree_qualifier", FLOPOANN.operand_degree_qualifier, "unmodified"),
        ("value_qualifier", FLOPOANN.operand_value_qualifier, "exact"),
        ("frequency_qualifier", FLOPOANN.operand_frequency_qualifier, "unspecified"),
    ):
        graph.add((iri, prop_, FLOPOANN[str(operand.get(key) or default)]))
    for key, prop_ in (
        ("operand_index", FLOPOANN.operand_index),
        ("text", FLOPOANN.operand_text),
        ("start", FLOPOANN.operand_start),
        ("end", FLOPOANN.operand_end),
        ("qualifier_text", FLOPOANN.operand_qualifier_text),
        ("qualifier_start", FLOPOANN.operand_qualifier_start),
        ("qualifier_end", FLOPOANN.operand_qualifier_end),
    ):
        _literal_annotation(graph, iri, prop_, operand.get(key))


def _emit_value_operands(graph: Graph, formal: URIRef, assertion: dict) -> int:
    """Emit ``flopoann:has_value_operand`` individuals; they never change the FAC class."""

    operands = [row for row in assertion.get("value_operands", []) or [] if isinstance(row, dict)]
    for position, operand in enumerate(operands):
        _emit_operand(
            graph,
            formal,
            FLOPOANN.has_value_operand,
            URIRef(f"{formal}/value-operand/{position}"),
            operand,
        )
    return len(operands)


def _emit_bearer_scope(graph: Graph, formal: URIRef, assertion: dict) -> bool:
    """Positional-scope provenance (E3) as annotations; the logic is in the FAC class."""

    scope = assertion.get("bearer_scope")
    if isinstance(scope, dict) and scope:
        for key, prop in (
            ("outer_bearer", FLOPOANN.bearer_scope_outer_bearer),
            ("scope_class", FLOPOANN.bearer_scope_class),
        ):
            iri = _curie_iri(scope.get(key))
            if iri is not None:
                graph.add((formal, prop, iri))
        for key, prop in (
            ("mode", FLOPOANN.bearer_scope_mode),
            ("scope_text", FLOPOANN.bearer_scope_text),
            ("scope_start", FLOPOANN.bearer_scope_start),
            ("scope_end", FLOPOANN.bearer_scope_end),
        ):
            _literal_annotation(graph, formal, prop, scope.get(key))
    stack = list(assertion.get("part_restrictions", []) or [])
    while stack:
        row = stack.pop()
        if isinstance(row, dict):
            _literal_annotation(graph, formal, FLOPOANN.part_text, row.get("part_text"))
            stack.extend(row.get("part_restrictions", []) or [])
    return bool(scope)


def _emit_qualitative_value_relation(graph: Graph, formal: URIRef, assertion: dict) -> URIRef:
    """Emit a reified annotation relation without inventing an OWL class expression."""

    relation = canonical_qualitative_relation(assertion)
    relation_iri = URIRef(str(formal) + "/qualitative-value-relation")
    interpretation = FLOPOANN[relation["interpretation"]]
    graph.add((relation_iri, RDF.type, OWL.NamedIndividual))
    graph.add((relation_iri, RDF.type, FLOPOANN.QualitativeValueRelation))
    graph.add((formal, FLOPOANN.has_qualitative_value_relation, relation_iri))
    graph.add((interpretation, RDF.type, OWL.NamedIndividual))
    graph.add((interpretation, RDF.type, FLOPOANN.QualitativeRelationInterpretation))
    graph.add((relation_iri, FLOPOANN.qualitative_relation_interpretation, interpretation))
    for key, prop in (
        ("from_value", FLOPOANN.qualitative_from_value),
        ("to_value", FLOPOANN.qualitative_to_value),
    ):
        value = URIRef(OBO + relation[key])
        graph.add((value, RDF.type, OWL.Class))
        graph.add((relation_iri, prop, value))
    for key, prop in (
        ("from_text", FLOPOANN.qualitative_from_text),
        ("from_start", FLOPOANN.qualitative_from_start),
        ("from_end", FLOPOANN.qualitative_from_end),
        ("connector_text", FLOPOANN.qualitative_connector_text),
        ("connector_start", FLOPOANN.qualitative_connector_start),
        ("connector_end", FLOPOANN.qualitative_connector_end),
        ("to_text", FLOPOANN.qualitative_to_text),
        ("to_start", FLOPOANN.qualitative_to_start),
        ("to_end", FLOPOANN.qualitative_to_end),
    ):
        _literal_annotation(graph, relation_iri, prop, relation[key])
    raw_relation = assertion.get("qualitative_value_relation") or {}
    for side, prop in (
        ("from", FLOPOANN.qualitative_from_operand),
        ("to", FLOPOANN.qualitative_to_operand),
    ):
        operand = raw_relation.get(f"{side}_operand")
        if isinstance(operand, dict):
            _emit_operand(
                graph, relation_iri, prop, URIRef(f"{relation_iri}/{side}-operand"), operand
            )
    return relation_iri
...
def _declare_vocabulary(graph: Graph) -> None:
    for cls in (
        ...
        FLOPOANN.QualitativeValueRelation,
        FLOPOANN.QualitativeRelationInterpretation,
        FLOPOANN.ValueOperand,
        SIO_STATEMENT,
        SIO_DOCUMENT,
    ...
    for prop in (
        ...
        FLOPOANN.has_qualitative_value_relation,
        FLOPOANN.qualitative_relation_interpretation,
        FLOPOANN.has_value_operand,
        FLOPOANN.qualitative_from_operand,
    ...
    *,
    ...
    with Path(gated_jsonl).open(encoding="utf-8") as handle:
        for line in handle:
            ...
            for index, assertion in enumerate(record.get("assertions", []) or []):
                ...
                qualitative_relation = bool(assertion.get("qualitative_value_relation"))
                structured_operands = not operands_fac_representable(assertion)
                description: URIRef | None = None
                if qualitative_relation or structured_operands:
                    if assertion.get("phenotype_class_iri"):
                        raise ValueError(
                            "qualitative value relations and approximated operands must not "
                            "carry a FAC class IRI"
                        )
                else:
                    description = URIRef(ensure_annotation_class_iri(assertion))
                    annotation_classes.add(str(description))
                    counts["fac_assertions"] += 1
                ...
                elif qualitative_relation:
                    _emit_qualitative_value_relation(graph, formal, assertion)
                    counts["structured_qualitative_relations"] += 1
                else:
                    counts["structured_value_operand_lists"] += 1
                if _emit_value_operands(graph, formal, assertion):
                    counts["assertions_with_value_operands"] += 1
                if _emit_bearer_scope(graph, formal, assertion):
                    counts["assertions_with_bearer_scope"] += 1
                if taxon is not None:
                ...
                if strict and taxon is not None and description is not None:
    ...
    return {
        **dict(counts),
        ...
        "reused_annotation_class_links": counts["fac_assertions"] - len(annotation_classes),
