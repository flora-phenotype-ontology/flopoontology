...
* existing ``EQ|PO|PATO``, relational ``EQR``, and ``PHENO|PO`` signatures reuse their registry IRI;
...
* relational EQ classes add nested ``has_part some (part and has_quality some Q)`` fillers
"""
...
from flopo2.ids.registry import IdAllocator, load_registry, relational_signature
...
class Candidate:
    ...
    def eq_signature(self) -> str:
        if self.part_restrictions:
            quality = (
                self.value_term_ids[0]
                if self.value_operator == "atomic" and len(self.value_term_ids) == 1
                else self.pato_id
            )
            return relational_signature(self.po_id, quality, self.part_restrictions)
        if self.value_operator == "atomic" and len(self.value_term_ids) == 1:
...
def load_candidates(
    ...
    support: Counter[CandidateKey] = Counter()
    assertion_iris: dict[CandidateKey, list[str]] = defaultdict(list)
    ...
    with Path(gated_jsonl).open(encoding="utf-8") as fh:
        for line in fh:
            ...
            for assertion_index, a in enumerate(obj.get("assertions", []) or []):
                ...
                parts = tuple(
                    (
                        row["property"].replace(OBO, ""),
                        row["filler_class"].replace(OBO, ""),
                        tuple(value.replace(OBO, "") for value in row["qualities"]),
                    )
                    for row in canonical_part_restrictions(a)
                )
                key = (po_id, effective_pato_id, operator, values, parts)
    ...
    return [
        Candidate(
            ...
            supporting_assertion_iris=tuple(
                dict.fromkeys(assertion_iris[(po, pato, operator, values, parts)])
            ),
            ...
            part_restrictions=parts,
        )
        for (po, pato, operator, values, parts), count in sorted(support.items())
    ]
...
def build_ontology(
    ...
    # OWLAPI emits a declaration for this referenced parent in Functional Syntax.
    # Carry its canonical annotation so the standalone candidate remains label-complete.
    g.add(
        (
            ANATOMICAL_ENTITY_PHENOTYPE,
            RDFS.label,
            Literal("plant anatomical entity phenotype", lang="en"),
        )
    )
    g.add((PUBESCENCE_PHENOTYPE, RDF.type, OWL.Class))
    g.add((PUBESCENCE_PHENOTYPE, RDFS.subClassOf, ANATOMICAL_ENTITY_PHENOTYPE))
    g.add((PUBESCENCE_PHENOTYPE, RDFS.label, Literal("pubescence phenotype", lang="en")))
    g.add(
        (
    ...
    emitted_flopo_iris: set[str] = {str(ROOT_CLASS), str(PUBESCENCE_PHENOTYPE)}
    ...
    for c in candidates:
        ...
        if c.pato_id == BOTANICAL_PUBESCENCE_ID:
            g.add((eq_iri, RDFS.subClassOf, PUBESCENCE_PHENOTYPE))
        ...
        nested_part_restrictions = []
        for prop, part_filler, part_qualities in c.part_restrictions:
            part_expression = _intersection(
                g,
                [
                    _obo(part_filler),
                    *[
                        _restriction(g, HAS_QUALITY, _obo(part_quality))
                        for part_quality in part_qualities
                    ],
                ],
            )
            nested_part_restrictions.append(_restriction(g, _obo(prop), part_expression))
            g.add((_obo(part_filler), RDF.type, OWL.Class))
            for part_quality in part_qualities:
                g.add((_obo(part_quality), RDF.type, OWL.Class))
        filler = _intersection(
            g, [_obo(c.po_id), *quality_restrictions, *nested_part_restrictions]
        )
