...
from flopo2.annotation.positional import load_bspo_labels
...
def _label(assertion: dict, po_labels: dict[str, str], pato_labels: dict[str, str]) -> str:
    ...
    part_phrase = _part_label(assertion.get("part_restrictions", []) or [], po_labels, pato_labels)
    if part_phrase:
        phrase = f"{phrase} with {part_phrase}"
    stages = [
...
def _part_label(rows: list, po_labels: dict[str, str], pato_labels: dict[str, str]) -> str:
    """Human-readable ``with <part> <qualities>`` phrase; labels are not part of identity."""

    bspo_labels = load_bspo_labels()
    phrases = []
    for row in rows:
        if not isinstance(row, dict):
            continue
        filler = str(row.get("filler_class", "") or "").replace(":", "_")
        label = po_labels.get(filler) or bspo_labels.get(filler) or filler
        qualities = [
            pato_labels.get(str(value).replace(":", "_"), str(value))
            for value in row.get("qualities", []) or []
        ]
        phrase = f"{label} {' and '.join(qualities)}".strip()
        nested = _part_label(row.get("part_restrictions", []) or [], po_labels, pato_labels)
        if nested:
            phrase = f"{phrase} with {nested}"
...
def build_annotation_extension(
    ...
    *,
    ...
    assertions = fac_assertions = structured_relations = structured_operands = records = 0
                ...
                for assertion in record.get("assertions", []) or []:
                    ...
                    if assertion.get("qualitative_value_relation"):
                        if assertion.get("phenotype_class_iri"):
                            raise ValueError(
                                "qualitative value relation must not carry a FAC class IRI"
                            )
                        structured_relations += 1
                        continue
                    if not operands_fac_representable(assertion):
                        # An approximated operand (``suborbicular``) is not entailed by its head
                        # value, so the assertion has no FAC class expression (E2).
                        if assertion.get("phenotype_class_iri"):
                            raise ValueError(
                                "an assertion with a non-entailing operand must not carry a FAC "
                                "class IRI"
                            )
                        structured_operands += 1
                        continue
    ...
    if include_imports:
        ...
        graph.add((ontology, OWL.imports, BSPO_IMPORT))
    ...
    return {
        ...
        "fac_assertions": fac_assertions,
        "structured_qualitative_relations": structured_relations,
        "structured_value_operand_lists": structured_operands,
        "annotation_classes": len(representatives),
        "reused_class_links": fac_assertions - len(representatives),
