...
_OBO_PREFIXES = {"PO", "PATO", "UO", "ENVO", "FLOPO", "RO", "BFO", "BSPO"}
_OBO_ID = re.compile(r"^(PO|PATO|UO|ENVO|FLOPO|RO|BFO|BSPO)_([A-Za-z0-9_.-]+)$")
# Total depth of the has-part grammar: bearer > part > part (schema extension E3).
MAX_PART_DEPTH = 2
...
def _canonical_part_rows(rows: Any, depth: int) -> list[dict[str, Any]]:
    unique: dict[str, dict[str, Any]] = {}
    for row in rows or []:
        if not isinstance(row, dict):
            raise ValueError(f"part restriction must be an object: {row!r}")
        prop = canonical_iri(row.get("property"))
        if prop != HAS_PART_IRI:
            raise ValueError(
                "part restriction property must be BFO:0000051 has part: "
                f"{row.get('property')!r}"
            )
        filler = canonical_iri(row.get("filler_class"))
        if not filler.startswith((OBO + "PO_", OBO + "FLOPO_", OBO + "BSPO_")):
            raise ValueError(
                "part restriction filler must be a PO class, a reviewed FLOPO-local "
                f"anatomical support class, or a pinned BSPO class: {row.get('filler_class')!r}"
            )
        qualities = sorted({canonical_iri(value) for value in (row.get("qualities", []) or [])})
        invalid = [
            value
            for value in qualities
            if not value.startswith((OBO + "PATO_", OBO + "FLOPO_"))
        ]
        if invalid:
            raise ValueError(
                "part restriction qualities must be PATO or FLOPO classes: "
                f"{invalid!r}"
            )
        nested_rows = row.get("part_restrictions", []) or []
        if nested_rows and depth >= MAX_PART_DEPTH:
            raise ValueError(
                f"part restriction depth exceeds {MAX_PART_DEPTH} (bearer > part > part)"
            )
        nested = _canonical_part_rows(nested_rows, depth + 1) if nested_rows else []
        if not qualities and not nested:
            raise ValueError(
                "part restriction must contain at least one quality or a nested part restriction"
            )
        ...
        # Additive key, present only for nested parts, so every depth-one FAC identity is
        # byte-for-byte unchanged.  Evidence fields (part_text/part_start/part_end) are provenance
        # and never enter the canonical form.
        if nested:
            canonical["part_restrictions"] = nested
        key = json.dumps(canonical, sort_keys=True, separators=(",", ":"))
        unique[key] = canonical
    return [unique[key] for key in sorted(unique)]


def canonical_part_restrictions(assertion: dict[str, Any]) -> list[dict[str, Any]]:
    """Canonicalize the supported nested part grammar for identity and OWL rendering."""

    return _canonical_part_rows(assertion.get("part_restrictions", []) or [], 1)
...
def annotation_class_signature(assertion: dict[str, Any]) -> dict[str, Any]:
    ...
    if assertion.get("qualitative_value_relation"):
        raise ValueError(
            "qualitative value relations are structured flora annotations, not FAC class "
            "expressions"
        )
    ...
    if part_restrictions:
        signature["part_restrictions"] = part_restrictions
