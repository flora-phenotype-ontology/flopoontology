...
from flopo2.ingest import collenette, fdac, florml, kew, ncvc_guide
...
def iter_path(target: Path) -> Iterator[TextSegment]:
    ...
    elif "ncvc" in target.name.lower() and target.suffix.lower() == ".jsonl":
        yield from ncvc_guide.iter_segments(target)
