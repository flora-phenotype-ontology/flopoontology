...
class PartRestrictionProperty(str, Enum):
    """
    Object properties allowed in the initial nested relational phenotype grammar.
    """
...
class QualitativeRelationInterpretation(str, Enum):
    """
    Annotation-level semantics for a grounded pair of qualitative values. These values are not OWL logical operators and must never be compiled as one_of or multiple is_a axioms.
    """
    continuum = "continuum"
    taxon_level_alternatives = "taxon_level_alternatives"
...
class ValueQualifier(str, Enum):
    """
    Controlled exactness or approximation of the asserted value. Approximation values are non-entailing: "nearly orbicular" and "suborbicular" are not orbicular, so an approximated operand never contributes its head value to a FAC union.
    """
    exact = "exact"
    approximately = "approximately"
    nearly = "nearly"
    ...
    sub = "sub"
    """
    Latin sub-/semi- prefix on a value ("subglabrous", "subcordate", "suborbiculaire"). Used only when no reviewed PATO/FLOPO term exists for the prefixed form; the head value is recorded as the operand value.
    """


class DegreeQualifier(str, Enum):
    """
    Controlled degree qualifier, orthogonal to statement frequency. Every value except completely is entailing for categorical values: X qualified by it is still an instance of X, so the head value may be used in a FAC expression and the cue is kept as provenance. The axis annotation groups values that describe the same dimension; the intensity values and the width, density, extent, texture and depth values can be combined.
    """
    ...
    narrowly = "narrowly"
    """
    Outline relatively long for its width ("narrowly elliptic", "étroitement").
    """
    broadly = "broadly"
    """
    Outline relatively wide for its length ("broadly ovate", "largement").
    """
    densely = "densely"
    """
    High density of the covering or structures ("densely pubescent", "densément").
    """
    sparsely = "sparsely"
    """
    Low density ("sparsely hairy", "éparsement", "peu ramifié").
    """
    shortly = "shortly"
    """
    Short extent of the qualified feature ("shortly acuminate", "brièvement").
    """
    finely = "finely"
    """
    Fine texture or thin elements ("finely pubescent", "finement").
    """
    deeply = "deeply"
    """
    Great depth of incision or curvature ("deeply cordate", "profondément").
    """
    shallowly = "shallowly"
    """
    Small depth of incision ("shallowly lobed", "peu profondément").
    """


class BearerScopeMode(str, Enum):
    """
    Compilation of a positional or sub-part cue.
    """
    substituted_bearer = "substituted_bearer"
    """
    anatomical_entity is the more specific PO part (e.g. leaf lamina abaxial epidermis).
    """
    part_restriction = "part_restriction"
    """
    The outer organ stays the bearer; the part is a part_restrictions filler.
    """
...
class TraitAssertion(ConfiguredBaseModel):
    ...
    value_operands: Optional[list[ValueOperand]] = Field(default=None, description="""Source-ordered operands of the categorical value expression, each with its own verbatim evidence and optional degree, approximation, or frequency qualifier (\"glabrous or sparsely pubescent\"). Every operand names one atomic grounded value. Operands never enter the FAC signature: when every operand carries at most an entailing degree or a frequency qualifier, the set of operand values equals value_terms (or the atomic quality) and the FAC class is unchanged. An operand with a non-entailing approximation (approximately, nearly, almost, sub) makes the assertion non-FAC-representable, so it then carries no phenotype_class_iri. When absent, value_terms alone keeps its meaning.""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion']} })
    bearer_scope: Optional[BearerScope] = Field(default=None, description="""Provenance of a positional or sub-part restriction of the named organ, such as \"beneath\", \"à la face supérieure\", \"at the apex\" or \"stellate-\". The logical content lives in anatomical_entity (bearer substituted by a more specific PO part such as leaf lamina abaxial epidermis) or in part_restrictions; this record keeps the outer organ and the exact cue so the source statement can be verified. It never participates in the FAC signature.""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion']} })
    qualitative_value_relation: Optional[QualitativeValueRelation] = Field(default=None, description="""A source-ordered relation between two grounded categorical values. This captures a qualitative continuum, explicit taxon-level variation, or a temporal transition without pretending that the endpoints form a finite OWL union. It is a structured annotation and is deliberately excluded from FAC and reusable FLOPO class materialization.""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion']} })
    ...
    part_restrictions: Optional[list[PartRestriction]] = Field(default=None, description="""Qualities borne by a specifically typed anatomical part of this assertion's bearer. These compile as nested has-part restrictions and must never be flattened into qualities of the outer bearer. The list is canonicalized and sorted for stable FAC identity.""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion', 'PartRestriction']} })
    ...
    value_high_inclusive: Optional[bool] = Field(default=True, description="""Whether value_high is an inclusive bound. True (default) renders xsd:maxInclusive; False renders xsd:maxExclusive for a strict upper bound such as the French participial \"n'atteignant pas 0,5 mm\" (reaching less than 0.5 mm). Only meaningful when value_high is set. Distinct FAC identity from an otherwise identical inclusive bound.""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion'], 'ifabsent': 'True'} })
    ...
    frequency_qualifier: Optional[FrequencyQualifier] = Field(default=FrequencyQualifier.unspecified, description="""Frequency force of the flora statement; unspecified preserves an absent cue.""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion', 'ValueOperand'],
    ...
    value_qualifier: Optional[ValueQualifier] = Field(default=ValueQualifier.exact, description="""Exactness or approximation of a categorical or numerical value.""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion', 'ValueOperand'],
         'ifabsent': 'ValueQualifier(exact)'} })
    degree_qualifier: Optional[DegreeQualifier] = Field(default=DegreeQualifier.unmodified, description="""Degree cue kept separate from frequency and epistemic modality.""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion', 'ValueOperand'],
...
class PartRestriction(ConfiguredBaseModel):
    """
    ...
    qualities: Optional[list[str]] = Field(default=None, description="""PATO or FLOPO quality classes borne by the typed part. May be empty only when nested part_restrictions are present (\"stellate-pubescent beneath\": abaxial epidermis has part trichome that is star shaped); the validator requires qualities or nested parts.""", json_schema_extra = { "linkml_meta": {'domain_of': ['PartRestriction']} })
    part_restrictions: Optional[list[PartRestriction]] = Field(default=None, description="""Nested typed parts of this part, maximum total depth two (bearer > part > part). Canonicalized and sorted like the top-level list; emitted in the FAC signature only when present, so every depth-one FAC identity is unchanged.""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion', 'PartRestriction']} })
    part_text: Optional[str] = Field(default=None, description="""Exact source cue naming the part, e.g. \"hairs\", \"beneath\", \"stellate-\", \"au sommet\". Provenance only; never part of the FAC signature.""", json_schema_extra = { "linkml_meta": {'domain_of': ['PartRestriction']} })
    part_start: Optional[int] = Field(default=None, description="""Zero-based inclusive offset of part_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['PartRestriction']} })
    part_end: Optional[int] = Field(default=None, description="""Zero-based exclusive offset of part_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['PartRestriction']} })


class BearerScope(ConfiguredBaseModel):
    """
    ...
    mode: BearerScopeMode = Field(default=..., description="""How the scope was compiled into the class expression.""", json_schema_extra = { "linkml_meta": {'domain_of': ['BearerScope']} })
    scope_text: str = Field(default=..., description="""Exact positional cue, e.g. \"beneath\", \"à la face inférieure\".""", json_schema_extra = { "linkml_meta": {'domain_of': ['BearerScope']} })
    scope_start: int = Field(default=..., description="""Zero-based inclusive offset of scope_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['BearerScope']} })
    scope_end: int = Field(default=..., description="""Zero-based exclusive offset of scope_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['BearerScope']} })


class ValueOperand(ConfiguredBaseModel):
    """
    ...
    text: str = Field(default=..., description="""Exact source text of the whole operand including qualifier cues.""", json_schema_extra = { "linkml_meta": {'domain_of': ['ValueOperand']} })
    start: int = Field(default=..., description="""Zero-based inclusive offset of text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['SourceStatement',
                       'UnresolvedTraitSpan',
                       'ValueOperand',
                       'SeasonContext',
                       'DevelopmentalStageContext',
                       'TermMention']} })
    end: int = Field(default=..., description="""Zero-based exclusive offset of text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['SourceStatement',
                       'UnresolvedTraitSpan',
                       'ValueOperand',
                       'SeasonContext',
                       'DevelopmentalStageContext',
                       'TermMention']} })
    degree_qualifier: Optional[DegreeQualifier] = Field(default=DegreeQualifier.unmodified, description="""Degree cue applying to this operand only.""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion', 'ValueOperand'],
         'ifabsent': 'DegreeQualifier(unmodified)'} })
    value_qualifier: Optional[ValueQualifier] = Field(default=ValueQualifier.exact, description="""Approximation cue applying to this operand only (±, nearly, sub-).""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion', 'ValueOperand'],
         'ifabsent': 'ValueQualifier(exact)'} })
    frequency_qualifier: Optional[FrequencyQualifier] = Field(default=FrequencyQualifier.unspecified, description="""Frequency cue applying to this operand only (\"or rarely hairy\").""", json_schema_extra = { "linkml_meta": {'domain_of': ['TraitAssertion', 'ValueOperand'],
         'ifabsent': 'FrequencyQualifier(unspecified)'} })
    qualifier_text: Optional[str] = Field(default=None, description="""Exact qualifier cue(s) for this operand, e.g. \"narrowly\", \"± densely\". Provenance only; it lies inside the operand text.""", json_schema_extra = { "linkml_meta": {'domain_of': ['ValueOperand']} })
    qualifier_start: Optional[int] = Field(default=None, description="""Zero-based inclusive offset of qualifier_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['ValueOperand']} })
    qualifier_end: Optional[int] = Field(default=None, description="""Zero-based exclusive offset of qualifier_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['ValueOperand']} })


class QualitativeValueRelation(ConfiguredBaseModel):
    """
    ...
    to_value: str = Field(default=..., description="""Grounded PATO or reviewed FLOPO value at the right/source-second endpoint. For a temporal transition this is the later state.""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
    from_operand: Optional[ValueOperand] = Field(default=None, description="""Optional structured left endpoint carrying per-endpoint qualifiers (\"narrowly linear\"). Its value must equal from_value and its text/offsets must equal from_text/from_start/from_end.""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
    to_operand: Optional[ValueOperand] = Field(default=None, description="""Optional structured right endpoint; same rules as from_operand.""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
    from_text: str = Field(default=..., description="""Exact source text denoting from_value.""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
    from_start: int = Field(default=..., description="""Zero-based inclusive offset of from_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
    from_end: int = Field(default=..., description="""Zero-based exclusive offset of from_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
    connector_text: str = Field(default=..., description="""Exact connector text supporting the interpretation, for example \"to\" or \"becoming\".""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
    connector_start: int = Field(default=..., description="""Zero-based inclusive offset of connector_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
    connector_end: int = Field(default=..., description="""Zero-based exclusive offset of connector_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
    to_text: str = Field(default=..., description="""Exact source text denoting to_value.""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
    to_start: int = Field(default=..., description="""Zero-based inclusive offset of to_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
    to_end: int = Field(default=..., description="""Zero-based exclusive offset of to_text in the source segment.""", json_schema_extra = { "linkml_meta": {'domain_of': ['QualitativeValueRelation']} })
...
PartRestriction.model_rebuild()
BearerScope.model_rebuild()
ValueOperand.model_rebuild()
QualitativeValueRelation.model_rebuild()
