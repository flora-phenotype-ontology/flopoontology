...
def _register_annotation_class(conn: sqlite3.Connection, assertion: dict) -> str | None:
    ...
    if (
        ...
        # Cardinality and qualitative value relations are retained as structured annotations but
        # are not equivalent to a currently supported OWL phenotype class expression.
        return None
        ...
        for line in fh:
            ...
            for a in obj.get("assertions", []) or []:
                ...
                values = {
                    ...
                    "qualitative_value_relation": json.dumps(
                        a.get("qualitative_value_relation") or {}, ensure_ascii=False
                    ),
                    "bearer_context_qualities": json.dumps(
                        a.get("bearer_context_qualities", []), ensure_ascii=False
                    ),
                    "part_restrictions": json.dumps(
                        a.get("part_restrictions", []), ensure_ascii=False
                    ),
                    "value_operands": json.dumps(
                        a.get("value_operands", []) or [], ensure_ascii=False
                    ),
                    "bearer_scope": json.dumps(
                        a.get("bearer_scope") or {}, ensure_ascii=False
                    ),
