...
_CURATOR_DECISIONS = {"accept", "accept_with_revision", "defer", "reject"}
...
def _proposal_index(
    path: Path, evidence_manifest: Path, *, curator_reviewed: bool = False
    ...
    for line_number, row in enumerate(rows, start=2):
        ...
        if curator_reviewed:
            if row["curator_decision"].strip() not in _CURATOR_DECISIONS:
                raise ValueError(
                    f"{path}:{line_number}: curator-reviewed table needs a known "
                    f"curator_decision, got {row['curator_decision']!r}"
                )
        elif row["curator_decision"].strip() or row["curator_notes"].strip():
            raise ValueError(
...
def build_curation_table(
    ...
    *,
    ...
    Machine proposal tables must leave curator fields blank. A table whose decisions a curator
    has recorded (``curator_reviewed``) must instead carry a known decision on every row.
    """
    mapping_rows = _read_tsv(mapping_report)
    aliases, proposals = _proposal_index(
        proposals_path, evidence_manifest, curator_reviewed=curator_reviewed
    )
    ...
    summary: dict[str, object] = {
        ...
        "policy": {
            "curator_decision_default": "curator-recorded" if curator_reviewed else "blank",
...
def main() -> None:
    ...
    parser.add_argument(
        "--curator-reviewed",
        action="store_true",
        help="The proposal table carries recorded curator decisions (e.g. after 2026-09-18).",
    )
    ...
    rows, summary = build_curation_table(
        ...
        curator_reviewed=args.curator_reviewed,
    )
