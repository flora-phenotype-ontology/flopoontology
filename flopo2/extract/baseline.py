...
from flopo2.extract.leaflet_context import leaflet_guarded
from flopo2.extract.surface_context import surface_guarded
...
def extract_segment_with_unresolved(
    ...
    for measurement in parse_measurements(text, obj.get("language", "")):
        ...
        if not po_id or leaflet_guarded(po_id, text, measurement.start):
            # A leaf-part bearer inside a leaflet description has no PO leaflet-part term.
            unresolved.append(_unresolved(
                measurement.source_text,
                measurement.start,
                measurement.end,
    ...
    for group in _quality_disjunction_groups(text, quality_matches):
        ...
        po_id = group[0][2]
        if leaflet_guarded(po_id, text, first_match.start()) or surface_guarded(
            po_id, family, text, first_match.start(), last_match.end()
        ):
            # Leave the members to the atomic loop, which keeps each span unresolved.
            continue
    ...
    for cue, match, po_id in quality_matches:
        ...
        if not reason and (
            not po_id
            or leaflet_guarded(po_id, text, match.start())
            # ``limbe glabre en dessus, pubescent en dessous``: one surface, not the organ.
            or surface_guarded(
                po_id, QUALITY_FAMILY.get(cue.pato_id, ""), text, match.start(), match.end()
            )
        ):
