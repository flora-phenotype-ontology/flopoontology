...
from flopo2.extract.baseline import QUALITY_FAMILY
...
def recover_negation_record(
    ...
    for span in record.get("unresolved_spans", []) or []:
        ...
        surface_rows = [
            ...
            if surface_guarded(
                r.po_id,
                QUALITY_FAMILY.get(r.pato_id, ""),
                str(record.get("text", "") or ""),
                int(span["start"]),
                int(span["end"]),
            )
        ]
        if surface_rows and len(surface_rows) == len(promote_rows):
            # ``glabre en dessous``: a one-surface value is not a value of the whole organ.
            retained.append(span)
            audit.append(
                _audit_row(
                    record, span, rows=surface_rows, disposition="held",
                    reason="surface_restricted_value",
                )
            )
            continue
        promote_rows = [r for r in promote_rows if r not in surface_rows]
        if not promote_rows:
