...
def recover_record(
    ...
    for unresolved in record.get("unresolved_spans", []) or []:
        ...
        if (
            disposition.startswith("recovered")
            and bearer is not None
            and leaflet_guarded(bearer.po_id, text, int(unresolved.get("start", 0) or 0))
        ):
            # The lamina described here belongs to a leaflet; PO has no leaflet apex/base term.
            disposition = "leaflet_context_bearer_withheld"
        if (
            disposition.startswith("recovered")
            and bearer is not None
            and surface_guarded(
                bearer.po_id,
                QUALITY_FAMILY.get(str(unresolved.get("candidate_pato_id", "") or ""), ""),
                text,
                int(unresolved.get("start", 0) or 0),
                int(unresolved.get("end", 0) or 0),
            )
        ):
            # ``glabre en dessus``: a one-surface value is not a value of the whole organ.
            disposition = "surface_restricted_value_withheld"
